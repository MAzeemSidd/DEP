...
//Get list of Posts
router.get('/', (req, res) => {
    const query = req.query.id ? `SELECT * FROM posts WHERE post_id=${req.query.id}` : 'SELECT * FROM posts'
...
router.put('/', (req, res) => {
    ...
    //else
    const query = 'UPDATE posts SET `user_id` = ?, `title` = ?, `content` = ? WHERE post_id = ?';
    const values = [
        req.body.user_id,
        req.body.title,
        req.body.content,
